...
def createFPlot(df, pic, ms=100):
    dic=sortMatrix2(df, pic, ms)
    rows = len(dic)
    cols = 0
    keys = dic.keys()
    for k in keys:
    ...
    plt.errorbar(x, np.mean(fMatrix, 0), yerr=np.sqrt(np.var(l, 0)), color='g')
    plt.xlim([-1, 35])
    #plt.ylim([-100, 200])
    plt.title('Summarized activity in 100 ms timeframes (' + str(pic) + ')')
    plt.xlabel('Timeframe ' + str(ms) + 'ms' )
    plt.ylabel('Frequency (Hz) + SD')
    #plt.show()
    plt.savefig('dis3sum' + pic, dpi=100)
    #return fMatrix
...
for key in np.unique(df3['target']):
    try:
        createFPlot(df3, key, 100)
    except IndexError:
        pass
